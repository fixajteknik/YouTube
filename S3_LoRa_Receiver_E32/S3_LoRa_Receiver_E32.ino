...
** Uncomment the "//" comment mark next to the module you are using and comment out the other module 
...
#define Address 2              // You can enter a value between 0 and 65000. Should be DIFFERENT from other modules.
#define Channel 20             // Frequency value should be the SAME as other modules.
#define DestinationAddress 1  // Address of the LoRa to which the message will be sent
...
void loop() {
  while (FixajSerial.available() > 1) {
    ...
    if (strcmp(data.password, "Fixaj.com") == 0) {
      ...
      Serial.println(*(float*)(data.Coor), 6);
      Serial.print("Big number: ");
      Serial.println(*(int*)(data.Number));
    }
  }
}
